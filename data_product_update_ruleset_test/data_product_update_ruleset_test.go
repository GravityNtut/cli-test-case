...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	ctx.Given(`^Create "'(.*?)'" ruleset "'(.*?)'" and enabled is "'(.*?)'"$`, ut.CreateDataProductRuleset)
	ctx.Given(`^Store NATS copy of existing data product "'(.*?)'" ruleset "'(.*?)'"$`, SaveRuleset)
	ctx.When(`^Update data product "'(.*?)'" ruleset "'(.*?)'" using parameters "'(.*?)'" "'(.*?)'" "'(.*?)'" "'(.*?)'" "'(.*?)'" "'(.*?)'" "'(.*?)'"$`, UpdateRulesetCommand)
	...
	ctx.Then(`^Use NATS jetstream to query the "'(.*?)'" "'(.*?)'" update successfully and parameters are "'(.*?)'" "'(.*?)'" "'(.*?)'" "'(.*?)'" "'(.*?)'" "'(.*?)'" "'(.*?)'"$`, ValidateRulesetUpdate)
	// ctx.Then(`^The error message should be "'(.*?)'"$`, AssertErrorMessages)
	ctx.Then(`^Use NATS jetstream to query the "'(.*?)'" "'(.*?)'" without changing parameters$`, ValidateRulesetNotChange)
}
